...
const inverseReadableMap = Object.keys(readableMap).reduce(function (
  inverse,
  key
) {
...
const cliCredsCheck = () => {
  ...
  return new Promise(resolve => {
    const missingCreds = Object.keys(readableMap).reduce(
      (missing, key) =>
    ...
    if (missingCreds.length > 0) {
      ...
      console.log(
        chalk.white(
          `Please enter your Slack Credentials. See https://git.io/vwLRq for help.`
        )
      )
      prompt.message = ''
      prompt.start()
    ...
    console.log()
    if (promptResults) {
      Object.keys(promptResults).forEach(resultKey =>
        conf.set(inverseReadableMap[resultKey], promptResults[resultKey])
      )
    }
...
const getToken = () => conf.get(SLACK_TOKEN_KEY)
