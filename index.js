...
const bot = new Slack({ token, nice: 1 })
...
const run = () => {
  ...
  creds
    ...
    .then(slackCredentials => {
      d.i(
        `credentials: ` +
          chalk.blue(JSON.stringify(slackCredentials, null, '  '))
      )

      // Cute loading messages
      loading.start()

      // Append credentials to environment so that slack api can access them.
      Object.keys(slackCredentials).forEach(
        key => (process.env[key] = slackCredentials[key])
      )
    ...
    .catch(err => {
      ...
      loading.immediateStop().then(() => {
        if (err.message === 'invalid_auth') {
          console.log(
            `\n${chalk.red.inverse(
              ' Slack API authentication failed. Please re-enter credentials. '
            )}\n`
          )
          creds.clear()
          run()
...
const start = (bot, id) => {
  ...
  Promise.all([slack.user.getPresence(commonArgs), slack.dnd.info(commonArgs)])
    ...
    .then(data => {
      ...
      utils.handleDnd(
        STATE,
        dndMessage.dnd_enabled,
        dndMessage.next_dnd_start_ts,
        dndMessage.next_dnd_end_ts
      )
